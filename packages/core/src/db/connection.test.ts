...
test("the upgrade path reclaims what the orphan purge frees", () => {
  const db = createTestDb();

  // Put the database back in the state an older lore left it in: migration 030
  // not yet applied, and an embedding for every chunk that has since gone.
  db.run("DELETE FROM _migrations WHERE name = '030_orphan_embeddings'");
  const blob = new Uint8Array(3072 * 4);
  ...
  // What Engine.dbFor does on the first open after an upgrade.
