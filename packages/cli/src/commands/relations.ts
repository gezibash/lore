...
export async function relationsSetCommand(
  ...
  const result = await client.setConceptRelation(fromConcept, toConcept, relationType, { weight });
  console.log(
...
export async function relationsUnsetCommand(
  ...
  const result = await client.unsetConceptRelation(fromConcept, toConcept, { relationType });
  if (result.removed === 0) {
...
export async function relationsListCommand(
  ...
  const relations = await client.listConceptRelations(opts);
