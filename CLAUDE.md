...
| 10  | Deep research                    | 9/10  | Full thread reconstruction via `trail(delta)` tool returns all entries with positions, statuses, topics. ask() trail entries show `entry_index` (e.g. "3/8") so agent sees where each match falls. Gap to 10: no narrative-level intent search (FTS needed), no cross-narrative linking (related investigations) |
